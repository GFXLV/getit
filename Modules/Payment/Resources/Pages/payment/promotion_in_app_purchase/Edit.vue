<template>
    <Head :title="$t('edit_promotion_in_app_purchase')" />
    <ps-layout>
                    ...
                    <form @submit.prevent="handleSubmit(inAppPurchase.id)">
                        <div class="grid w-full sm:w-1/2 gap-6">
                            <div>
                                ...
                                <ps-input ref="in_app_purchase_prd_id" type="text" v-model:value="form.in_app_purchase_prd_id"
                                    :placeholder="$t('promotion_in_app_purchase_product_id')"
                                ...
                                <ps-label-caption textColor="text-red-500 " class="mt-2 block">{{errors.in_app_purchase_prd_id}}
                                </ps-label-caption>
                            </div>

                            <div>
                                <ps-label class="text-base">{{$t('payment__be__description')}}</ps-label>
                                <ps-textarea rows="4" v-model:value="form.description" :placeholder="$t('payment__be_description')" />
                            </div>
                            ...
                            <div>
                                ...
                                <ps-input ref="day" type="text" v-model:value="form.day" :placeholder="$t('payment__be_day')"
                                    @keypress="onlyNumber" @keyup="validateEmptyInput('day', form.day)" @blur="validateEmptyInput('day', form.day)" />
...
export default defineComponent({
    ...
    layout: PsLayout,
    props: ['errors', 'inAppPurchase', 'day_attribute', 'type_attribute', 'status_attribute', 'types'],
    setup(props) {
        ...
        function handleSubmit(id) {
            this.$inertia.post(route("promotion_in_app_purchase.update", id), form, {
                forceFormData: true,
                onBefore: () => {
                    loading.value = true;
                },
        ...
        return { day, onlyNumber, type, in_app_purchase_prd_id, validateInAppPurchasePrdIdInput, validateEmptyInput, handleSubmit, ps_action_modal, form, loading, success, ps_danger_dialog, ps_image_icon_modal }
    },
    computed: {
        breadcrumb() {
            return [
                ...
                {
                    label: trans('payment__be_edit_promotion_iap'),
