<template>
    ...
    <ps-layout>
        ...
        <ps-table2 :row="row" :search="search" :object="this.inAppPurchases" :colFilterOptions="colFilterOptions"
            ...
            <template #button>
                    <ps-button v-if="can.createItem" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" /> {{ $t('core__be_add_package_in_app_purchase') }}</ps-button>
            </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const columns = [
            ...
            {
                label: trans('payment__iap_post_count'),
                field: props.countKey,
                type: 'Integer',
                action: 'Action'
            },
            {
                label: trans('payment__iap_price'),
                field: props.priceKey,
                type: 'Integer',
                action: 'Action'
            },
            {
            ...
            {
                label: trans('payment__iap_currency'),
        ...
        function confirmDeleteClicked(id) {
            ps_danger_dialog.value.openModal(
                ...
                trans('payment__delete_package_iap_info'),
    ...
    methods: {
        handleCreate() {
            this.$inertia.get(route("package_in_app_purchase.create"));
        },
        handleEdit(id){
            this.$inertia.get(route('package_in_app_purchase.edit',id));
        },
        handlePublish(id,hasPermission){
            if(hasPermission){
                this.$inertia.put(route('package_in_app_purchase.statusChange',id));
            }
