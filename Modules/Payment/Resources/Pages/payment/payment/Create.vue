<template>
    <Head :title="$t('core__be_add_payment')" />
    <ps-layout>
        <!-- breadcrumb start -->
        ...
        <ps-card class="w-full h-auto">
            <div class="rounded-xl">
                ...
                <div class="bg-primary-50 py-2.5 ps-4 ">
                    <ps-label-header-6 textColor="text-secondary-800 dark:text-secondary-100">{{$t('payment__be_payment_info')}}</ps-label-header-6>
                </div>
                <!-- card header end -->
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function handleSubmit() {
            this.$inertia.post(route("payment.store"), form, {
                forceFormData: true,
                onBefore: () => {
                    loading.value = true;
                },
        ...
        return { description, name, validateDescriptionInput, validateNameInput, handleSubmit, form, loading, success };
    },
    computed: {
        breadcrumb() {
            ...
            return [
                ...
                {
                    label: trans('core__be_add_payment'),
