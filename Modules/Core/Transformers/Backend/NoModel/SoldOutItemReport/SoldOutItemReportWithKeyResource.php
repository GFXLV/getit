...
namespace Modules\Core\Transformers\Backend\NoModel\SoldOutItemReport;

use Illuminate\Http\Resources\Json\JsonResource;
use Illuminate\Support\Str;
use Modules\Chat\Transformers\Backend\Model\UserBoughtWithKeyResource;
use Modules\Core\Constants\Constants;
use Modules\Core\Entities\CustomizeUiDetail;
use Modules\Chat\Entities\ChatHistory;

class SoldOutItemReportWithKeyResource extends JsonResource
{
    public function toArray($request)
    {
        ...
        $totalOffer = 0;
        foreach ($this->user_boughts as $user_bought){
            $offer_amount = 0;
            $conds['item_id'] = $user_bought->item_id;
            $conds['seller_user_id'] = $user_bought->seller_user_id;
            $conds['buyer_user_id'] = $user_bought->buyer_user_id;
            $chat = ChatHistory::where($conds)->first();
            if($chat){
                $offer_amount = $chat->nego_price;
            }
            $totalOffer = $totalOffer + $offer_amount;
        }
        ...
        return [
            ...
            'added_user_id@@email' => isset($this->owner) && !empty($this->owner) ? $this->owner->email : '',
            ...
            'user_boughts'=> UserBoughtWithKeyResource::collection($this->whenLoaded('user_boughts')),
            'owner'=> isset($this->owner)?$this->owner:[],
            'authorizations' => authorizationWithoutPolicy(Constants::soldOutItemReportModule)        
        ] + $changedCustomFieldFormat;            
    }
}
