<template>
    <Head :title="$t('edit_shipping')" />
    <ps-layout>
        <div class="py-6">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white  shadow-xl sm:rounded-lg">
                    <div class="mt-6 ml-6 mx-auto flex justify-center">
                        <form @submit.prevent="handleSubmit(this.shipping.id)">
                            <ps-label-header-4>{{$t('edit_shipping')}}</ps-label-header-4>
                            <div>
...
export default defineComponent({
    ...
    layout: PsLayout,
    props: ['errors', 'shipping', 'shops'],
    data() {
        return {
            form: useForm(
                {
                    name: this.shipping.name,
                    shop_id: this.shipping.shop_id,
                    price: this.shipping.price,
                    days: this.shipping.days,
                    status: this.shipping.status == 1 ? true : false,
                    "_method": "put"
                }
            )
        }
    ...
    setup(props) {
        ...
        return { validateEmptyInput, validateNameInput, validatePriceInput, onlyNumbeWithCustom }
    },
    methods: {
        handleSubmit(id) {
            this.$inertia.post(route('shipping.update', id), this.form,{forceFormData: true});
        },
    },
})
</script>
