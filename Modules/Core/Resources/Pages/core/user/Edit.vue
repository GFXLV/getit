<template>
    <Head :title="$t('core__be_edit_user')" />
    <ps-layout>
        <!-- breadcrumb start -->
                        ...
                        <div class="grid w-full sm:w-1/2 gap-6">
                            ...
                            <div v-for="(coreField, index) in coreFieldFilterSettings.filter((coreField) => coreField.original_field_name === 'email' && coreField.enable === 1 && coreField.is_delete === 0)" :key="index">
                                <div class="flex flex-row justify-between items-end">
                                    <ps-label class="text-base">{{ $t(coreField.label_name) }} <span v-if="coreField.mandatory=1" class="text-red-800 font-medium ms-1">*</span></ps-label>
                                    <ps-label @click="verifyEmailClicked" class="mb-0.5 text-sm cursor-pointer" textColor="text-primary-500"> <span v-if="emailVerified">Verified </span><span v-else>Verify </span></ps-label>
                                </div>
                            ...
                            <div v-for="(coreField, index) in coreFieldFilterSettings.filter((coreField) => coreField.original_field_name === 'user_phone' && coreField.enable === 1 && coreField.is_delete === 0)" :key="index">
                                <div class="flex flex-row justify-between items-end">
                                    <ps-label class="text-base">{{ $t(coreField.label_name) }} <span v-if="coreField.mandatory=1" class="text-red-800 font-medium ms-1">*</span></ps-label>
                                    <ps-label @click="verifyPhoneClicked" class="mb-0.5 text-sm cursor-pointer" textColor="text-primary-500"> <span v-if="phoneVerified">Verified </span><span v-else>Verify </span></ps-label>
                                </div>
                            ...
                            <div v-if="user.verify_types.includes(2) || user.verify_types.includes(5) || user.verify_types.includes(3)" class="flex flex-col space-y-5">
                                <div class="flex flex-row">
                                    <div v-if="user.verify_types.includes(2)" class="p-1 rounded-md border border-secondary-200 flex flex-row items-center flex-grow-0">
                                        <ps-icon name="bluemark" w='20' h='20' viewBox='0 0 20 20' class="text-green-500" />
                                        <ps-label class="mx-2">{{ $t('core__be_verified_with_google') }}</ps-label>
                                        <img :src="$page.props.sysImageUrl + '/google_logo.jpg'" class="w-6 h-6" alt="google logo" />
                                    </div>
                                </div>
                                <div class="flex flex-row">
                                    <div v-if="user.verify_types.includes(5)" class="p-1 rounded-md border border-secondary-200 flex flex-row items-center flex-grow-0">
                                        <ps-icon name="bluemark" w='20' h='20' viewBox='0 0 20 20' class="text-green-500" />
                                        <ps-label class="mx-2">{{ $t('core__be_verified_with_apple') }}</ps-label>
                                        <img :src="$page.props.sysImageUrl + '/apple_logo.png'" class="w-6 h-6" alt="google logo" />
                                    </div>
                                </div>
                                <div class="flex flex-row">
                                    <div v-if="user.verify_types.includes(3)" class="p-1 rounded-md border border-secondary-200 flex flex-row items-center flex-grow-0">
                                        <ps-icon name="bluemark" w='20' h='20' viewBox='0 0 20 20' class="text-green-500" />
                                        <ps-label class="mx-2">{{ $t('core__be_verified_with_facebook') }}</ps-label>
                                        <img :src="$page.props.sysImageUrl + '/facebook_logo.jpg'" class="w-6 h-6" alt="google logo" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-for="(coreField, index) in coreFieldFilterSettings.filter((coreField) => coreField.original_field_name === 'user_about_me' && coreField.enable === 1 && coreField.is_delete === 0)" :key="index">
                                <ps-label class="text-base">{{ $t(coreField.label_name) }} <span v-if="coreField.mandatory=1" class="text-red-800 font-medium ms-1">*</span></ps-label>
                                <ps-textarea rows="4" v-model:value="form.user_about_me" :placeholder="$t(coreField.placeholder)"
                            ...
                            <div v-for="(custom_field_header) in customizeHeaders" :key="custom_field_header.id">
                                ...
                                <div class="mb-4" v-if="custom_field_header.ui_type_id === 'uit00009' && custom_field_header.enable === 1 && custom_field_header.is_delete === 0">
                                    ...
                                    <div v-if="user.image" class="flex items-end pt-4">
                                        <img 
                                         v-lazy=" { src: $page.props.uploadUrl + '/' + user.image.img_path, loading: $page.props.sysImageUrl+'/loading_gif.gif', error: $page.props.sysImageUrl+'/default_profile.png' }"
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const emailVerified = ref(props.user.verify_types.includes(1));
        const phoneVerified = ref(props.user.verify_types.includes(4));
        ...
        let form = useForm(
                {
                    ...
                    verify_types: props.user.verify_types,
                    user_cover_photo: "",
        ...
        function handleSubmit(id) {
            ...
            if(form.verify_types.includes(2)){
                verifyArray.push(2);
            }
            if(form.verify_types.includes(5)){
                verifyArray.push(5);
            }
            if(form.verify_types.includes(3)){
                verifyArray.push(3);
            }
            ...
            form.verify_types = verifyArray.toString();
        ...
        function verifyEmailClicked(){
            let msg = trans('core__be_confirm_add_email_verify');
            if(emailVerified.value){
                msg = trans('core__be_confirm_remove_email_verify');
            }
            ps_confirm_dialog.value.openModal(trans('conf_modal_label'),
                msg,
                trans('core__be_btn_confirm'),
                trans('btn_cancel'),
                ()=>{
                    emailVerified.value = !emailVerified.value;
                },
                ()=>{});
        }

        function verifyPhoneClicked(){
            let msg = trans('core__be_confirm_add_phone_verify');
            if(phoneVerified.value){
                msg = trans('core__be_confirm_remove_phone_verify');
            }
            ps_confirm_dialog.value.openModal(trans('conf_modal_label'),
                msg,
                trans('core__be_btn_confirm'),
                trans('btn_cancel'),
                ()=>{
                    phoneVerified.value = !phoneVerified.value;
                },
                ()=>{});
        }
    ...
    computed: {
        breadcrumb() {
            ...
            return [
                ...
                {
                    label: trans('core__be_edit_user'),
