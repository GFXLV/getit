<template>
    <Head :title="$t('core__create_phone_country_code')" />
    <ps-layout>
        <!-- breadcrumb start -->
        ...
        <ps-card class="w-full h-auto">
            <div class="rounded-xl">
                ...
                <div class="bg-primary-50 dark:bg-primary-900 py-2.5 ps-4 rounded-t-xl">
                    <ps-label-header-6 textColor="text-secondary-800 dark:text-secondary-100">{{ $t('core__country_code_info') }}
                    </ps-label-header-6>
                </div>
                <!-- card header end -->
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function handleSubmit() {
            this.$inertia.post(route('phone_country_code.store'), form, {
                forceFormData: true,
        ...
        return {
            validateCountryCodeInput, validateEmptyInput, handleSubmit, form,country_name,
            country_code,
            loading,
            success }
    },
    computed: {
        breadcrumb() {
            return [
                ...
                {
                    label: trans('core__create_phone_country_code'),
