        ...
        <ps-card class="w-full h-auto">
            <div class="rounded-xl">
                ...
                <div class="bg-primary-50 dark:bg-primary-900 py-2.5 ps-4 rounded-t-xl">
                    <!-- <ps-label-header-6 >{{$t('core__be_user_info')}}</ps-label-header-6> -->
                    <ps-label-header5>{{$t('core__be_user_info')}}</ps-label-header5>
                </div>
                <!-- card header end -->
                    ...
                    <form @submit.prevent="handleSubmit(user.id)">
                        <div class="grid w-full sm:w-1/2 gap-6">
                            ...
                            <div>
                                ...
                                <ps-label-title-3 v-if="!user.user_cover_photo">{{ $t('core__be_recommended_size_400_200') }}</ps-label-title-3>
                                <div v-if="user.user_cover_photo" class="flex items-end pt-4">
                                    <img v-lazy=" { src: $page.props.uploadUrl + '/' + user.user_cover_photo, loading: $page.props.sysImageUrl+'/loading_gif.gif', error: $page.props.sysImageUrl+'/default_profile.png' }" class="w-96 h-48" :alt="$t(core__be_user_cover_photo)" />
                                    <ps-button type="button" :disabled="true" @click="replaceImageClicked(user.user_cover_photo.id)" rounded="rounded-full" shadow="drop-shadow-2xl" class="-ms-10 mb-2" colors="bg-white text-primary-500 dark:bg-secondaryDark-black" border="border border-1 dark:border-secondary-700 border-secondary-300" padding="p-1.5" hover="" focus="">
                                ...
                                <ps-image-upload :disabled="true" v-else uploadType="image" v-model:imageFile="form.user_cover_photo" />
                            ...
                            <div v-for="(custom_field_header) in customizeHeaders" :key="custom_field_header.id">
                                ...
                                <div class="mb-4" v-if="custom_field_header.ui_type_id === 'uit00009' && custom_field_header.enable === 1 && custom_field_header.is_delete === 0">
                                    ...
                                    <div v-if="user.image" class="flex items-end pt-4">
                                        <img v-lazy=" { src: $page.props.uploadUrl + '/' + user.image.img_path, loading: $page.props.sysImageUrl+'/loading_gif.gif', error: $page.props.sysImageUrl+'/default_profile.png' }"
        ...
        breadcrumb() {
            ...
            return [
                ...
                },
                {
                    ...
                    url: route('user_report.index'),
                },
                {
                    label: trans('core__be_user_report_detail'),
