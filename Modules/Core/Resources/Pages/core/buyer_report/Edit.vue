<template>
    ...
    <Head :title="$t('core__be_buyer_details')" />
    <ps-layout>
        ...
        <ps-card class="w-full h-auto">
            <div class="rounded-xl">
                ...
                <div class="bg-primary-50 dark:bg-primary-900 py-2.5 ps-4">
                    <ps-label-header-6 textColor="text-secondary-800 dark:text-secondary-100">{{ $t('core__be_buyer_info_lbl') }}</ps-label-header-6>
                </div>
                <!-- card header end -->
                ...
                <div class="px-4 pt-6 dark:bg-backgroundDark">
                    <form>
                        <div class="grid w-full sm:w-1/2 gap-6">
                            <div>
                                <ps-label>{{ $t('user_name_label') }}</ps-label>
                            ...
                            <div>
                                <ps-label>{{ $t('core__be_address_label') }}</ps-label>
                                <ps-textarea :disabled="true" rows="3" v-model:value="form.user_address" :placeholder="$t('core__be_address_label')"></ps-textarea>
                            </div>
                            <div>
                                <ps-label class="mb-1">{{ $t('core__be_role') }}</ps-label>
                                <ps-input :disabled="true" type="text" v-model:value="form.role_id" :placeholder="$t('core__be_role')"/>
                            </div>
                            <div>
                                <ps-label>{{ $t('core__be_about_me') }}</ps-label>
                                <ps-textarea :disabled="true" rows="3" v-model:value="form.user_about_me" :placeholder="$t('core__be_about_me')"></ps-textarea>
                            </div>
        ...
        breadcrumb() {
            return [
                ...
                },
                {
                    ...
                    url: route('buyer_report.index'),
                },
                {
                    label: trans('core__be_buyer_details'),
