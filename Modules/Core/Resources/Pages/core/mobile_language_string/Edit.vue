<template>
    <Head :title="$t('core__be_edit_mobile_language_string')" />
    <ps-layout>
        <!-- breadcrumb start -->
        ...
        <ps-card class="w-full h-auto">
            <div class=" rounded-xl ">
                ...
                <div class="bg-primary-50 dark:bg-primary-900 py-2.5 ps-4 rounded-t-xl">
                    <ps-label-header-6 textColor="text-secondary-800 dark:text-secondary-100">{{$t('edit_language_string')}}</ps-label-header-6>
                </div>
                <!-- card header end -->

                <!-- card body start -->
                <div class="px-4 pt-6 dark:bg-backgroundDark">
                    <form @submit.prevent="handleSubmit(this.mobile_language_string.id)">
                        <div class="grid w-full sm:w-1/2 gap-6">
                            ...
                            <div>
                                ...
                                <ps-input :disabled="true" type="text" v-model:value="form.key" :placeholder="$t('core__be_key_placeholder')"
...
export default defineComponent({
    ...
    layout: PsLayout,
    props: ['errors', 'mobile_language_string', 'mobile_language'],
    data() {
        return {
            form: useForm(
                {
                    key: this.mobile_language_string.key,
                    value: this.mobile_language_string.value,
                    language_id: this.mobile_language.id,
                    "_method": "put"
                }
            )
        }
    ...
    setup(props) {
        ...
        let form = useForm(
            {
                key: props.mobile_language_string.key,
                value: props.mobile_language_string.value,
        ...
        function handleSubmit(id) {
            this.$inertia.post(route('mobile_language_string.update', [props.mobile_language, id]), form, {
                forceFormData: true,
    ...
    computed: {
        breadcrumb() {
            ...
            return [
                ...
                {
                    label: trans('core__be_edit_mobile_language_string'),
