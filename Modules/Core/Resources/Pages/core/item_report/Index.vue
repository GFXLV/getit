<template>
    <Head :title="$t('item_report_module')" />
    <ps-layout>
         <!-- breadcrumb start -->
        ...
        <ps-table2 ::row="row" :search="search" :object="this.items" :colFilterOptions="colFilterOptions"
            ...
            <template #searchLeft>
                <a :href="route('item_report.csv.export')" class="font-medium transition duration-150 ease-in-out px-2 py-1.75 ms-1 rounded text-primary-500 border border-primary-500 hover:outline-none hover:ring hover:ring-blue-100 focus:outline-none focus:ring focus:ring-blue-300 opacity-100 flex items-center"><ps-icon name="fileUpload" class="mx-0.5 font-semibold" />{{ $t('core__be_export_btn') }}</a>
            </template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        // For data table
        const showFilter = props.selectedCategory || props.selectedSubcategory || props.selectedDealOption || props.selectedPurchaseOption || props.selectedDate ? ref(true): ref(false);
        ...
        const columns = [
            ...
            {
                label: trans('core__be_item_condition'),
                field: props.itmConditionOfItem + '@@name',
                type: "String",
            },
            {
        ...
        function handleSearchingSorting(page = null, row = null) {
            Inertia.get(route('item_report.index'),
                {
    ...
    methods: {
        handleDetail(id){
            this.$inertia.get(route('item_report.show',id));
        },
        FilterOptionshandle(value) {
            Inertia.put(route('item_report.screenDisplayUiSetting.store'),
                {
                    value,
