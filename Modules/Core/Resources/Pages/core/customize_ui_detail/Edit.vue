<template>
    <Head :title="$t('core__edit_attribute')" />
    <ps-layout>
        <!-- breadcrumb start -->
            ...
            <div class="rounded-xl">
                <!-- card header start -->
                 <div class="bg-primary-50 dark:bg-primary-900 py-2.5 ps-4 rounded-t-xl">
                    <ps-label-header-6 textColor="text-secondary-800 dark:text-secondary-100">{{$t('core__edit_attribute')}}</ps-label-header-6>
                </div>
                <!-- card header end -->
                ...
                <div class="px-4 pt-6 dark:bg-backgroundDark">
                    <form @submit.prevent="handleSubmit()">
                        <div class="grid w-full sm:w-1/2 gap-6">
                            ...
                            <div>
                                <ps-label class="text-base mb-2">{{$t('core__api_token_label')}}<span class="text-red-800 font-medium ms-1">*</span></ps-label>
...
export default defineComponent({
    ...
    layout: PsLayout,
    props: ['errors', 'customizeHeader', 'customizeDetail', 'tableId'],
    setup(props) {
        ...
        let form = useForm({
            ...
            core_keys_id: props.customizeHeader.core_keys_id,
            name: props.customizeDetail.name,
            _method: "put"
        })

        function handleSubmit() {
            this.$inertia.post(route('attribute.update', [props.tableId, props.customizeHeader.core_keys_id, props.customizeDetail.id]), form, {
                forceFormData: true,
        ...
        return { validateEmptyInput,handleSubmit, loading, success, name, form }
    },
    computed: {
        breadcrumb() {
            ...
            return [
                ...
                },
                {
                    ...
                    url: route('attribute.index', [this.tableId, this.customizeHeader.core_keys_id])
                },
                {
                    label: trans('core__edit_attribute'),
    ...
    methods: {
        handleCancel() {
            this.$inertia.get(route('attribute.index', [this.tableId, this.customizeHeader.core_keys_id]));
        },
    },
})
</script>
