<template>
    <Head :title="$t('core__be_edit_user_role')" />
    <ps-layout>
        <!-- breadcrumb start -->
...
export default defineComponent({
    ...
    setup(props) {
        ...
        for(let i=0; i<props.modules.length; i++){
            ...
            for(let j=0; j<props.rolePermissions.length; j++){

                if(props.rolePermissions[j].module_id == props.modules[i].id ){
                    booleanPermission[props.modules[i].id] =props.rolePermissions[j].permission_id.split(',');
                    form.permissions[props.modules[i].id]= props.rolePermissions[j].permission_id;
                }
            }
        ...
        function handleSubmit() {
            this.$inertia.post(route('user_role.update', props.role.id), form, {
                forceFormData: true,
    ...
    computed: {
        breadcrumb() {
            ...
            return [
                ...
                {
                    label: trans('core__be_edit_user_role'),
