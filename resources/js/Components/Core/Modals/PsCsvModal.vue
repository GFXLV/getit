<template>
    <ps-modal ref="psmodal" line="hidden" maxWidth="480px" bodyHeight="574px" :isClickOut='false' theme="px-4 pt-4 pb-8 shadow-xl rounded-lg bg-white dark:bg-secondaryDark-black" >
        <template #title>
            ...
            <ps-label-header-4 class="text-center font-normal text-2xl mt-6 mb-4"> {{ $t("ps_csv_modal__title") }} </ps-label-header-4>

        </template>

        <template #body class="overflow-hidden">
            <div class="mx-2.5">
                <ps-draggable class=" mt-4 px-2.5" @drop.prevent="drop">
                    ...
                    <input type="file" accept=".csv" ref="image" style="display: none;"  @change="onCsvSelected($event)">
