<template>
    <!-- Global Search start -->
    <ps-table-search v-if="!searchHide" :perPageOptions="perPageOptions" :colFilterOptions="colFilterOptions">
        <!-- for search left -->
        <template #searchLeft>
        ...
        <template #searchRight>
            ...
            <ps-input-with-right-icon class="w-60 h-10 rounded-full flex " v-model:value="searchterm" :placeholder="globalSearchPlaceholder" >
                <template #icon >
    ...
    <table class="table-auto w-full " :class="margin">
        <thead class="bg-primary-500 text-white dark:text-black text-2xl justify-content" >
            <tr>

                <th v-for="column in columns.filter(col => col.field == 'action')" :key="column.field" v-show="colFilterOptions.filter((item) => item.key == column.field).length == 0
                                    ? true
                ...
                <th v-for="column in columns.filter(col => col.field != 'action')" :key="column.field"
                    ...
                    : !colFilterOptions.filter((item) => item.key == column.field)[0].hidden"
                >
                    <ps-label v-if="column.field!='action'" textColor="flex text-secondary-50 dark:text-secondary-900 font-semibold my-2 ms-3 items-center">
                        {{ $t(column.label) }}
...
export default {
    ...
    props: {
        'margin': { type: String, default: 'mt-6 sm:mt-8 lg:mt-10' },
